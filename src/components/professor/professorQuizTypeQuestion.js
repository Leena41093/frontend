...
import { addNewQuizQuestionAnswer, getQuizQuesionAnswerList, editQuizQuestion, addProfQuizImage, deleteProfQuizImage } from '../../actions/professorActions';

class ProfessorQuizTypeQuestion extends Component {
	constructor(props) {
		super(props);
		this.state = {
			questions_answer: {
	...
	componentDidMount() {
		...
		this.setState({ batchDeatail: batchDeatail, instituteId: this.props.instituteId });
	}
	...
	validate() {
		...
		let { questionArray, error } = this.state;
	...
	onSaveQuiz() {
		...
		if (!isValidForm) {
			...
			const pro1 = this.props ? this.props.location.state.data : "";
			let { questionArray } = this.state;

			const pro = this.props.location.state.data;
			let data = {
				payload: {
					questions_answer: questionArray,
					quiz_id: pro.quiz_id
			...
			this.props.addNewQuizQuestionAnswer(data).then(() => {
				let res = this.props.quizQuestionAnswer;

				if (res && res.data.status == 200) {
					infoToste("Quiz Questions Added Successfully");
	...
	onDeleteQuizQuestion(index, questionNo, event) {
	...
	onDeleteQuizQuestionImage(index, questionNo, event) {
		...
		this.setState({ imageAddBtn: true, questionArray })
		const pro = this.props.location.state.data;
		let data = {
			payload: {
				quiz_id: pro.quiz_id,
	...
	numberOfOption(index, errorProperty, event) {
		...
		if (event.target.value == 2) {
			...
			// errors[index] = { ...errors[index], option3: true }
			// errors[index] = { ...errors[index], option4: true }
			// delete questionArray[index].option3
		...
		this.setState({ questionArray, errors })
	}

	addNewQuestion() {
		let { isQuestionAdd } = this.state;
		let { questionArray, errors } = this.state;
		let questionObject = {
			question: '',
			// option1: '',
			// option2: '',
						...
						<div style={{ marginLeft: '40px' }} className="queList__header">
							<div className="divider-container nomargin">
										...
										{question.image_url ?
											<div>
												<div >
													...
													{/* <button onClick={this.onDeleteQuizQuestionImage.bind(this, index, question.question_no)} className="c-btn prime" style={{ border: "none", color: "white", backgroundColor: "#990000",  fontSize: "13px" }} >delete Image</button> */}
												</div>
												<div>
								...
								<div className="divider-block option__cont">
									<div className="form-group cust-fld">
										<label>Options</label>
										<input type="number" value={question.no_of_options} onChange={this.numberOfOption.bind(this, index, "isNoOfOptionVisible")} className="form-control" min="2" max="4" placeholder="Option" />
		...
		return (
			<div className="c-container clearfix">
				...
				<div className="clearfix">
					...
					<div className="divider-container">
						<div className="divider-block text--left">
							<span className="c-heading-lg">{pro1.data ? pro1.data.quiz_title : ""}</span>
						</div>
				...
				<div className="modal-footer">
					<div id="addbtn" className="clearfix text--right">
...
const mapStateToProps = ({ app, professor, auth }) => ({
	...
	quizQuestionAnswer: professor.quizQuestionAnswer,
...
export default connect(mapStateToProps, mapDispatchToProps)(ProfessorQuizTypeQuestion) 