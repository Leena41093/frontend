...
import { getStudentDetail, getIsProfessorAdmin } from '../../actions/index';
...
class FinanceDetail extends Component {
    ...
    if (pro != null) {
      ...
      this.props.getStudentDetail(data).then(() => {
        ...
        if (res && res.status == 200) {
          this.setState({ Student: res.data.response, profilePicture: res.data.response.profilePicture, student_id: pro.student_id });
        }
      });
    }
  ...
  onDeleteEntry(flag) {
    ...
    if (flag == 'deleteBatchStudFinance') {
  ...
  onDeleteBatch(id, event) {
    ...
    this.props.deleteStudentBatch(data).then(() => {
      ...
      if (res && res.status == 200) {
        ...
        this.props.getStudentDetail(datas).then(() => {
          let resstudentdetail = this.props.studentDetail;
          if (resstudentdetail && resstudentdetail.status == 200) {
            this.setState({ Student: resstudentdetail.data.response });
          }
        });
        ...
        if (Number(newPayment.amount) < Number(unPaidInstallments[0].amount)) {
          ...
          for (let i = 1; i < unPaidInstallments.length; i++) {
            if (remainingAmount > unPaidInstallments[i].amount) {
              ...
              this.props.updateStudentPaymentDetail(datapayload).then(() => {

              })
            }
            else {
              unPaidInstallments[i].amount = unPaidInstallments[i].amount - remainingAmount;
              let dataunpaidinstalments = {
                payload: {
                  student_finance_schedule_id: unPaidInstallments[i].student_finance_schedule_id,
                  payment_date: unPaidInstallments[i].payment_date,
                  installment_no: unPaidInstallments[i].installment_no,
              ...
              this.props.updateStudentPaymentDetail(dataunpaidinstalments).then(() => {
      ...
      this.props.createStudentPaymentSchedule(data).then(() => {
        ...
        if (res && res.status == 200) {
          ...
          this.props.createStudentPaymentDetail(datas).then(() => {
            let resstudentpaymentdetail = this.props.studentPaymentDetailAdd;
            if (resstudentpaymentdetail && resstudentpaymentdetail.status == 200) {
              let newArr = resstudentpaymentdetail.response;
              let onlyUnPaidAray = [];
              newArr.map((data) => {
                  ...
                  <li key={"key" + index} style={{ width: "280px" }}>
                    ...
                    <td>
                        <div className="card__elem__setting">
                          ...
                          <button onClick={this.deleteStudentPayment.bind(this, index, obj.student_finance_schedule_id)} className="act-delete"></button>
                        </div>
                      </td>
                    </tr>
                  </li>
        ...
        <div key={"batch" + index} className="c-batchList">
          ...
          <span className="c-batchList__title">{batch.className}<img src="/images/Arrow.png" alt="logo" style={{ height: "10px", width: "20px" }} />{batch.batchName}
            <div className="card__elem__setting1">
              ...
              <button style={{ marginRight: "-220px", marginTop: "-7px" }} data-toggle="modal" data-target="#quizSubmit" onClick={this.onDeleteModel.bind(this, "deleteBatchStudFinance", 0, 0, batch.timeTable[0].batch_id)} className="act-delete pull-right"></button>
            </div>
        ...
        <div>
          <div className="c-card__form">
            <div className="divider-container">
              ...
              <div className="divider-block text--left">
                <div className="c-user-pic">
                  <div className="user--img"><img src={this.state.profilePicture ? this.state.profilePicture : "/images/avatars/Avatar_default.jpg"} alt="Avatar" /></div>
                </div>
              </div>
            </div>
            <div className="form-group static-fld">
      ...
      <div className="c-container clearfix" style={{ marginBottom: "100px" }}>
        ...
        <div className="clearfix">
          <div className="c-brdcrum">
            <a className="linkbtn hover-pointer" onClick={this.backButton.bind(this)} >Back to Finance Directory</a>
          </div>
          <div className="divider-container">
            <div className="divider-block text--left">
        ...
        <div className="c-container__data">
          ...
          <div className="card-container">
            ...
            <div className="c-card" style={{ height: "500px", overflow: "auto" }}>
              <div className="c-card__title">
